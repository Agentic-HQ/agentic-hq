...
- **Add an "## English Language Description" section** to the plan. Write a paragraph describing how the system will work using class/interface names **bolded** and verbs *italicized*. This serves as a readable design overview that validates the class structure makes sense as English. Example:
  > "When the CLI runs `agentic-hq list`, it *creates* a **DefaultWorkflowSearchResults** (no constructor parameters). The **DefaultWorkflowSearchResults** internally *creates* a **DefaultAhqWorkspace**, then *asks* it to `scanForWorkflows()`. The **DefaultAhqWorkspace** *locates* the AHQ workspace root (via `AGENTIC_HQ_WORKSPACE_ROOT` env var or git root), then *scans* the `.agentic-hq/plugins/*/skills/*/` directories for `ahq-workflow.json` files. For each file found, it *creates* a **DefaultAhqWorkflow** by passing the file path to its constructor. **DefaultAhqWorkflow** *reads* its JSON file and *stores* just the raw JSON string — no cached value objects. When a method like `getShortName()` is called, it *dynamically creates* a **DefaultWorkflowShortName** from the JSON on demand. Similarly, `getFullPath()` *dynamically creates* a **DefaultFullPath** (from a freshly created **DefaultPluginId** and **DefaultSkillId**), and `getExampleCommand()` *dynamically creates* a **DefaultExampleCommand** (from a freshly created **DefaultWorkflowShortName** and **DefaultExampleParameters**). Each value object *renders* itself via `toString()` at the output boundary. The **DefaultAhqWorkspace** *returns* an array of **AhqWorkflow** objects. Back in **DefaultWorkflowSearchResults**, `displayToUser()` *prints* the header, then *tells* each **AhqWorkflow** to *displayItself* — each workflow is responsible for rendering its own line using its contained **WorkflowShortName**, **FullPath**, **WorkflowDescription**, and **ExampleCommand**, which each *render* themselves via `toString()` at the output boundary."
