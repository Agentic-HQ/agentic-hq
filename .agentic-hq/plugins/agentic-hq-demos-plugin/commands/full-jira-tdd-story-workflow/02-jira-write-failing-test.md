...
- **Add an "## English Language Description" section** to the plan. Write a paragraph describing how the system will work, walking through the main scenarios step by step (start to finish). Use class/interface names **bolded** and *italicize* ONLY verbs that represent actual method calls between objects (e.g. *getWorkflowListingString*, *registerWorkflowsWith*). Do NOT italicize narrative verbs like "creates", "checks", "delegates to" — those are plain text describing flow, not method names. ANTI-PATTERN: "*delegatesToAWorkspaceImpl*" — this is narrative, not a method name. Correct: "delegates to a **WorkspaceImpl**". PHRASING: Use "asks X to *doThing*" not "asks X for its *doThing*" — reads more naturally as English. See `docs/dev/project-design-requirements.md` for full ELD formatting rules. Example:
