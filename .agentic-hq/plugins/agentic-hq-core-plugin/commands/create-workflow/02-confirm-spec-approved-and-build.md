...
3. **Reference / source files for the build** — **what you read here depends on the creation mode** (determined just below):
   ...
   **A. Simple workflow — math-workflow** (3 sequential pure-data-transform commands, no user interaction, output of one feeds input of next):
   ...
   - `{example-workflow-skill-dir}/ts-workflow/pnpm-workspace.yaml` — pnpm-workspace.yaml pattern (per-directory `packages: ['.']` + `minimumReleaseAge` + `frozenLockfile`, the AHQ-152 supply-chain standard). Copy this file **verbatim** into the new workflow in Step 4e. There is deliberately NO `.npmrc`: pnpm-only settings must live in this pnpm-only file — pnpm 11+ ignores them in `.npmrc`, and npm (which shares `.npmrc` but has no such settings) warns about them (AHQ-211).
...
4. **SKILL.md** — the byte-identical template that reports where the skill is installed (the Agentic HQ engine constructs and runs the workflow command itself, AHQ-210)
5. **package.json + pnpm-workspace.yaml + `.gitignore`** — the standard file set: `commander` dependency + `typescript`/`@types/node` devDependencies (**no** `agentic-hq` dependency, no tsx, no postinstall — the Workflow Build (2) links the framework); a minimal per-directory `pnpm-workspace.yaml` carrying `packages: ['.']`, `minimumReleaseAge` and `frozenLockfile` (the AHQ-152 supply-chain standard every workflow ships — pnpm-only settings live here, never in a `.npmrc`, AHQ-211); and a `.gitignore` for the build products (`node_modules/`, `dist/`)
...
**1. Copy (the Copy manifest).** Create the destination `{commands-dir}` and `{skills-dir}` first, then copy every file/dir the Plan lists from its absolute **source** dirs to those absolute **destination** dirs — a cross-root copy when the source lives in the AHQ install (the Plan's paths already account for that). Copy: every command `NN-*.md`, `SKILL.md` (**verbatim, byte-identical — no substitutions**), the whole `ts-workflow/` **source and config** (`src/`, `package.json`, `tsconfig.json`, `pnpm-workspace.yaml`, `.gitignore`, **and `pnpm-lock.yaml`**), any templates, and the `docs/` directory. **Exclude `node_modules/` and `dist/`** — both are build products the Workflow Build (2) recreates (`node_modules/` holds a now-wrong `agentic-hq` symlink; `dist/` holds the source workflow's compiled JS). Keeping `pnpm-workspace.yaml` + `pnpm-lock.yaml` preserves the frozen-lockfile supply-chain standard (AHQ-152, `frozenLockfile: true` in the workspace yaml) and a reproducible install. (If an OLD source workflow still carries a `.npmrc`, skip it — pnpm 11+ ignores its settings and npm warns about them, AHQ-211.)

> **The copied workflow runs in the new (often non-AHQ) workspace automatically.** The byte-identical `SKILL.md` template reports the skill's install location to the Agentic HQ engine, which constructs and runs the shared-workflow-runner command itself (AHQ-210), and the Workflow Build (2) (`scripts/build-workflow.cjs`, run by the runner in `build-first` mode) does `pnpm install`, creates the `node_modules/agentic-hq` link to the AHQ package root, and compiles into `dist/` — so the copy resolves the framework from any workspace and needs **no** dependency rewiring.

**2. Rewire (the Rewire manifest).** Apply the resolved target values from the Plan:
- Rename the CLI file `{source-workflow-id}-cli.ts` → `{workflow-id}-cli.ts` (the `<skill-id>-cli.ts` convention — the engine runs `dist/{skill-id}-cli.js`, deriving `skill-id` from the skill directory the template `SKILL.md` reports, so the filename must match the skill directory name).
...
**Copy** `{example-workflow-skill-dir}/SKILL.md` **verbatim** to `{skills-dir}/SKILL.md` — **never author or edit a SKILL.md**. Every workflow ships the same byte-identical template: it reports its own install location (`skill-base-dir`) to the Agentic HQ engine, which derives `skill-id` from the skill directory name and constructs and runs the shared-workflow-runner command on `dist/{skill-id}-cli.js` itself (AHQ-210), so the template needs **no** per-workflow substitutions. (That is also why the CLI filename must follow the `{workflow-id}-cli.ts` convention from 4c.) After copying, verify the copy is byte-identical to the source (e.g. compare `shasum` output).

### 4e. Create package.json, pnpm-workspace.yaml, .gitignore and tsconfig.json

> **`from-scratch` mode only.** In `from-existing` mode these were all copied (and `package.json` rewired — `name`) in Step 4-COPY — skip 4e.

Create the standard ts-workflow file set — `{skills-dir}/ts-workflow/package.json`, `pnpm-workspace.yaml`, `.gitignore` and `tsconfig.json` — following the math-workflow reference files (Step 1's list):
...
- **`pnpm-workspace.yaml`** — required under pnpm 11 so a plain `pnpm install` run in the `ts-workflow` directory treats it as its own workspace root (pnpm stops at the nearest `pnpm-workspace.yaml`). Copy it **verbatim** from the math-workflow reference — do not hand-write it. It carries `packages: ['.']`, `minimumReleaseAge` (the AHQ-152 supply-chain cooldown) and `frozenLockfile: true` (the AHQ-152 standard **every** workflow ships, so dependency-version changes stay deliberate and visible in `package.json` / `pnpm-lock.yaml` diffs). Nothing else — the standard dependency set needs no build-script approvals. Do **not** put these settings in a `.npmrc`: pnpm 11+ ignores pnpm settings there, and npm — which shares `.npmrc` but understands none of them — prints an "Unknown project config" warning on every command (AHQ-211). A freshly-scaffolded workflow has **no `pnpm-lock.yaml` yet**, and that's fine: pnpm only enforces `frozenLockfile` against an *existing, out-of-sync* lockfile, so the first `pnpm install` (run by the Workflow Build in Command 03's checks) installs cleanly and generates the lockfile; later installs are then frozen against it.
...
2. **Files written** — a flat list with absolute paths, grouped:
   ...
   - **Skill files**: `{ahq-workflow-metadata-filename}`, the TypeScript CLI, `{skills-dir}/SKILL.md`, the `ts-workflow/package.json`, the `ts-workflow/pnpm-workspace.yaml`, the `ts-workflow/.gitignore`, the `ts-workflow/tsconfig.json`.
