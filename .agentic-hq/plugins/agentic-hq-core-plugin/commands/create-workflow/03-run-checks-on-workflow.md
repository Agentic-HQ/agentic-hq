...
4. **SKILL.md exists and is byte-identical to the bundled template** (`{example-workflow-skill-dir}/SKILL.md`) — since AHQ-210 the SKILL.md contains nothing workflow-specific: it just reports `skill-base-dir` to the Agentic HQ engine, which derives the program name `dist/{workflow-id}-cli.js` from the skill directory name. So the CLI link to verify is the **filename convention**: the CLI file must be `{workflow-id}-cli.ts` (compiled to `dist/{workflow-id}-cli.js`), and any byte difference from the template SKILL.md is a failure
