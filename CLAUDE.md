...
- All validation and linting must pass before story completion (UPDATE: We'll be adding these checks in the workflow commands so that once this is done you won't have to "remember" to do this any more and this directive can be removed from CLAUDE.md. Also, since https://agentic-hq.atlassian.net/browse/AHQ-7 has been implemented, this is now enforced by the /commit command running "pnpm validate" which runs typescript validation, format checking, lint checking and unit tests)
