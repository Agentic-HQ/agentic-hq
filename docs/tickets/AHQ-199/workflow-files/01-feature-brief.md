...
The project's documentation matches the shipped npm reality: the README opens with a short npm/npx Quick Start for Normal Users, the full contributor setup lives in a new `docs/dev/setting-up-agentic-hq-for-development.md` linked from CONTRIBUTING.md, and every doc still written for the clone-era Quick Start is brought in line.

## User Story

**As a**: developer who's heard about Agentic HQ  
**I want:** a README Quick Start that gets me running workflows straight from npm  
**So that:** I can try the tool in minutes without cloning the repo — while contributors still get a complete, separate setup guide
...
IMPORTANT: In this Jira we are moving all the instructions for prerequisites and checking out the repo and getting agentic-hq-dev working, and a section clarifying the different users (UPDATE: this will be an Appendix called "The Two Types Of Agentic HQ User" in this doc, as it's not required information - just useful clarification) into a new:
...
AHQ-199 is the docs Sub-Task (Sub-Task 8) of AHQ-195: now that the whole npm pipeline works end-to-end (AHQ-196 → AHQ-209), rewrite the user-facing documentation around the **two user groups** — **Normal Users**, who `npm install -g agentic-hq` (or `npx`) and never clone the repo, and **Contributors**, who clone the repo and use `agentic-hq-dev`. Concretely: (1) the README Quick Start becomes a short npm-based install-and-run guide (prerequisites, `npm install -g agentic-hq`, folder-trust note, run `reversal`/`add-feature`/`create-workflow` via the `agentic-hq` binary), keeping the README's other sections and doc links; (2) all the clone/corepack/pnpm/`npm link`/`agentic-hq-dev` setup instructions move into a new `docs/dev/setting-up-agentic-hq-for-development.md`, which also contains the section clarifying the two user groups (UPDATE: this will be an Appendix called "The Two Types Of Agentic HQ User" in this doc, as it's not required information - just useful clarification), and is linked from a new section in CONTRIBUTING.md; (3) knock-on updates across the docs that reference the old clone-based Quick Start — most notably `troubleshooting-quickstart.md` (entirely clone-centric today), `ci-configuration.md` (maps CI steps to README step numbers), `docs/README.md` (index), CONTRIBUTING.md's existing "Local development setup" summary, and the glossary's root-model link retarget mandated by the parent brief.
...
**AI Note (appended 2026-08-22):** largely overtaken by events — **0.2.0 was published and registry-verified later this same session** (`dist-tags.latest` → 0.2.0; results recorded in AHQ-209's implementation summary under `## Publish Completion (2026-08-22)`), so the docs are simply written against the live 0.2.0. The only part still needing your answer: should stale version references (e.g. CONTRIBUTING's "currently v0.1.0") be updated to 0.2.0 as part of this ticket?
...
- `README.md` — the clone-based Quick Start to replace with the npm one; `agentic-hq-dev` example commands throughout; the run-from-AHQ-workspace TIP and section anchors need rework.
- `CONTRIBUTING.md` — "Local development setup" section to repurpose as the dev-doc link (Q3); stale "currently v0.1.0"; its CI section describes the contributor flow.
- `docs/user-docs/troubleshooting-quickstart.md` — entirely clone-centric (corepack, `pnpm install`, `npm link`); needs an npm-install-user path with contributor material following the setup instructions.
- `docs/tickets/AHQ-195/workflow-files/01-feature-brief.md` — parent brief: Sub-Task 8 instructions, split rationale; done-markers and sequence fix applied during this stage.
- `docs/tickets/AHQ-209/workflow-files/03-implementation-summary.md` — final migrated state plus the 0.2.0 Publish Completion record (registry facts for the docs).
- `docs/tickets/AHQ-208/workflow-files/03-implementation-summary.md` — two-builds architecture, `agentic-hq-dev` rename, and which docs its own §8 pass already updated.
- `docs/dev/ci-configuration.md` — "README step ↔ CI step" table must retarget to the new dev doc; says `npm link` installs "the `agentic-hq` binary" (stale name).
- `docs/glossary.md` — root-model explanation at line ~96 links to the AHQ-200 ticket file; retarget per the parent instruction (durable section in `how-agentic-hq-works.md`, or inline).
- `docs/README.md` — docs index: needs the new dev doc listed and audience framing adjusted.
- `docs/dev/how-agentic-hq-works.md` — current post-AHQ-208; candidate home for the durable two-roots section.
- `docs/user-docs/workflow-descriptions/overview-of-workflows.md` — already uses plain `agentic-hq`; cross-check only.
- `docs/dev/publish-checklist.md` — current after AHQ-209; reference for registry/publish facts, not a work item.
- `docs/tickets/AHQ-198/workflow-files/03-implementation-summary.md` — the 0.1.x registry verification matrix and the 0.1.0-deprecation/npx-crash facts to document.

## Acceptance Criteria

- A Normal User can go from nothing to a running workflow using only the README's npm Quick Start — no clone, no pnpm/corepack — with tool-user prerequisites (Claude Code; Linux node-pty build toolchain), the folder-trust note, and `agentic-hq` in every user-facing example.
- The new `docs/dev/setting-up-agentic-hq-for-development.md` holds the full contributor setup plus the appendix "The Two Types Of Agentic HQ User", and CONTRIBUTING.md's repurposed section links to it.
- No live doc still tells tool users to clone the repo, names the wrong binary for its audience, maps to the old README step numbers, or carries stale version text (0.2.0 is current).
- The glossary's two-roots explanation no longer depends on a per-ticket workflow artifact link.
- The AHQ-195 parent brief records AHQ-201/208/209 as done, with the corrected sub-task sequence (applied during this stage — verifiable in that doc).

## Brief Approval

The human read the finalized brief and approved it as-is at the Researcher's single AskUserQuestion approval gate on 2026-08-22 ("Approve brief" chosen; feature judged a good size for one run, no split suggested). The workflow continues to the Planner.
