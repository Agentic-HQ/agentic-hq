...
| 4 | [docs-from-conversation-about-tailcut-with-claude-web-agent/tailcut-benchmark-spec.md](docs-from-conversation-about-tailcut-with-claude-web-agent/tailcut-benchmark-spec.md) | **The deliverable spec both arms receive** (the arms get the stripped copy in [experiment-handoff/](experiment-handoff/)). Read before touching anything experiment-related. |
| 4b | [../13-experiment-protocol-and-judging-rubric.md](../13-experiment-protocol-and-judging-rubric.md) | **The frozen protocol and rubric** — run configuration, objective gates, traps, design-quality rubric, blind judging mechanics. Read before any run or any judging. |
| 4c | [../14-birgitta-ousterhout-full-build-kick-off-guidance.md](../14-birgitta-ousterhout-full-build-kick-off-guidance.md) | **Kick-Off Guidance (not a plan)** for building the `birgitta-ousterhout-full-build` workflow — the Ousterhout Design Principles List (§2), the Guides/Sensors mapping and the full-build stage shape. Read before anything touching that workflow. |
| 4d | [full-jo-research-notes.md](full-jo-research-notes.md) | The research behind doc 14 §2: every APoSD source with its provenance, all quotations in full and organised by principle, and the complete TDD dossier. Reference material — read on demand, or before rewording any Guide. |
...
Docs are numbered in reading/creation order, **01–15**: 01–14 exist today, and the plan creates 15 (comparison report). **Next free number: 15.**
