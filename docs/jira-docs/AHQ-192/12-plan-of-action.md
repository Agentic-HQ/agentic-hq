...
| 3 | 💻 | `birgitta-ousterhout-full-build` built via create-workflow (from scratch) under AHQ-193; pushed | 🔄 in progress — 3.1 sources read (both Böckeler articles + doc 13 + AHQ mechanics); 3.2 doc 14 written 2026-07-26, **rewritten 2026-07-27 around Steve's vertical-slice decision**, then extended the same day with **§2 The Ousterhout Design Principles List** — 16 named, sourced and quoted principles + the 14 APoSD red flags, with the Guides renamed to match and three added (G10–G12); full research pile in [supporting-docs/full-jo-research-notes.md](supporting-docs/full-jo-research-notes.md). **All five open questions in §10 now answered — Q5 (TDD vs Ousterhout) answered 2026-07-27: keep the ordering, drop the TDD label, the design drives the development; doc 14 §2.3 and §5.4 rewritten accordingly.** **Next: 3.3 — Steve's final read of doc 14, then `/git:02` freezes it.** |
