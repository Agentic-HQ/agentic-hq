...
- **Model parity:** AHQ passes **no `--model` flag** (verified — `claude-command-builder.ts` builds plugin-dir args, the allowed-tools flag and the command only), so arm 2 runs on the VM's configured default model. **Arm 1 is therefore also launched with no `--model` flag**, and parity holds by construction.
  ...
  **⚠️ SUPERSEDED 2026-08-04 (AM8): the actual model both arms run is `claude-fable-5[1m]`, not Opus 5.** Discovered at plan step 6.1: Steve had pinned `"model": "claude-fable-5[1m]"` in `~/.claude/settings.json` on 2026-08-04 at 20:25, **before the shutdown for `tailcut-01-baseline`**, so the pin is inside the baseline snapshot and both arms (and the judges) inherit the identical model — the parity mechanism ("no `--model` anywhere, both inherit the VM default") is intact; only the recorded value above is stale. Effort remains `high`. Additionally, both arms' workspace `.claude/settings.local.json` gains `"autoCompactWindow": 200000` (see AM8): with a 1M-context default, auto-compact would otherwise fire only near 1M tokens, deep in the degraded long-context regime; both arms instead compact at 200k. Doc 15 and the John email must say **"one-shot Fable 5 (high reasoning, 200k auto-compact window)"**.
...
| Item | Arm 1 | Arm 2 |
|---|---|---|
| Exact launch command line | **Written and frozen 2026-08-04** (5.4/5.5) → [`experiment-results/run-commands/arm-1-launch.md`](experiment-results/run-commands/arm-1-launch.md). `claude --allowedTools="Bash Edit Write MultiEdit"` + the nine `mcp__mcp-atlassian__*` tools; no `--model`; no `--dangerously-skip-permissions`. **Confirmed at 6.1/6.2** — launched per the file (6.1 ledger record); the AM8 exit/resume reused the same grant; one session transcript. | **Written and frozen 2026-08-04** → [`experiment-results/run-commands/arm-2-launch.md`](experiment-results/run-commands/arm-2-launch.md). `agentic-hq full-build -- --spec-file=./tailcut-benchmark-spec.md`, **run from `~/dev/claude/agentic-hq/tailcut`**. *(Confirm at 6.4.)* |
...
| Web tools granted (Q1 outcome) | **Granted** — `WebSearch`+`WebFetch` via workspace `.claude/settings.local.json` (5.3, AM8 revision). **Never used** — the session transcript contains zero `WebSearch`/`WebFetch` calls (checked at 6.2). | *(fill)* |
| Wall-clock start / end | **Pinned at 6.2:** the 6.1 "20:38" was **local time (BST)** — the session transcript's first event is 19:40:35 UTC = 20:40:35 BST. End = final commit `7949ee5`, authored 21:55:21 BST, push verified 20:55 UTC. **Total ≈ 1 h 16 m**, including the AM8 exit/resume (consistent with Steve's ≈ 1 h 20 m report). Transcript activity at 21:04 UTC afterwards is the post-run informal-analysis Q&A, not the run. | *(fill)* |
...
| **AM8** | 2026-08-04 | **Yes — arm 1 was ~72k tokens into its run** (the only amendment made after an arm started) | **Two environment facts, no gate/criterion/weight touched.** **(a) Model record corrected:** the VM default both arms inherit is **`claude-fable-5[1m]`**, not `claude-opus-5[1m]` — Steve had pinned it in `~/.claude/settings.json` at 20:25 on 2026-08-04, *before* the `tailcut-01-baseline` shutdown, so the pin is inside the baseline and parity-by-construction is unaffected; §2.2's 4.5-era record was stale. **(b) `"autoCompactWindow": 200000`** added to both arms' workspace `.claude/settings.local.json`: arm 1's edited mid-run at ~72k tokens (session exited and resumed with the same `--allowedTools` grant to pick it up); arm 2's re-applied after the restore via step 0 of `arm-2-launch.md`, since the baseline predates the change. The 5.3 md5 (`8404ac70…`) refers to the pre-AM8 file. | **Steve's decision, 2026-08-04.** With a 1M-context default, auto-compact fires only near 1M tokens — far past the point where long-context quality degrades — so an unattended arm would spend most of its run in the weakest part of the model's window. Compacting at 200k keeps both arms in the strong regime. Applied identically to both arms; incapable of favouring either on any Block A or Block N criterion. Arm 1's first ~72k tokens ran without the setting, which is immaterial: compaction would not have triggered below 200k under either configuration, so its behaviour to that point was identical. Disclose in doc 15 alongside the other AM entries. |

| **AM9** | 2026-08-04 | **Yes — after arm 1 completed, at capture (6.2)** | **§6 item 1 capture *form* only:** arm 1's raw per-rep data (`rpc.csv`, `bulk.jsonl`, nstat snapshots — ~41 MB, ~10 MB compressed) was **not committed** to this repo. The committed capture is the arm's curated `results/` set (report, chart, summary.json, per-rep `verify.txt` / `meta.json` / `tc_stats.txt` / `marker_dump.txt` — the set the arm itself chose to push) **plus** the arm's `run_all.log` (gitignored in the arm repo, so otherwise unpushed). The raw data remains available in the `tailcut-02-arm1-complete` snapshot, whose stated purpose is preserving the full end state. Same form to be used for arm 2 at 6.4. | Golden Rule 11 (Steve on a metered connection, 2026-08-04): a ~10 MB push is not "small and load-bearing" once §3.2's measurement-integrity comparison has already been performed in the VM against the raw data. No gate, criterion or weight touched. |
