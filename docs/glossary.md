...
supplies it), `tsconfig.json` (emits `src` → `dist`),
`pnpm-workspace.yaml` (which carries the pnpm settings incl. `frozenLockfile` —
there is no `.npmrc` since AHQ-211), `pnpm-lock.yaml`, `.gitignore`. `dist/` and
